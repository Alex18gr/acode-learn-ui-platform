...
export class ModalNewSectionComponent implements OnInit {
  ...
  @Output() editedCourseSection: EventEmitter<CourseSection> =
  ...
  onSaveChanges() {
    ...
    if (this.editMode) {
      ...
      this.instructorCoursesService
        .updateCourseSection(this.editingCourseSection, this.currentCourse)
        .subscribe(
          (data: CourseSection) => {
      ...
      this.instructorCoursesService
        .createCourseSection(tempCourseSection, this.currentCourse)
        .subscribe(
          (data: CourseSection) => {
            this.editedCourseSection.emit(data);
            this.toastService.addSaveToast(
              'Course Section Created',
              'Course Section "' + data.name + '" created successfully.'
            );
            this.savingChanges = false;
            this.hideModal();
          },
