...
export class EditSectionComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      changes &&
      changes.selectedCourseSection &&
      changes.selectedCourseSection.currentValue &&
      changes.selectedCourseSection.currentValue !==
        changes.selectedCourseSection.previousValue
    ) {
      if (!this.form) {
        this.createForm();
      }
  ...
  saveCourseSection() {
    const courseSectionToSave: CourseSection = _.cloneDeep(
    ...
    this.instructorCourseService
      .updateCourseSection(courseSectionToSave, this.currentCourse)
      .subscribe(
        (data: CourseSection) => {
          this.courseSectionSaved.emit(this.form.getRawValue());
          this.toastService.addSaveToast(
            'Course Section Updated',
            'Course Section "' + data.name + '" updated successfully.'
          );
