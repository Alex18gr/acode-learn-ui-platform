...
export class DynamicGuideComponent implements OnInit, OnChanges {
  ...
  editModal: DynamicGuideEditResourcesModalComponent;
  guideData: GuideData;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes) {
      if (
        changes.guideResource &&
        changes.guideResource.currentValue !==
          changes.guideResource.previousValue
      ) {
        this.initGuide();
      }
    }
  ...
  private initGuide() {
    this.dynamicGuideService
      .getGuideDataFromResource(this.guideResource)
  ...
  onSaveGuide(value: GuideData) {
    ...
    console.log(value);
    this.dynamicGuideService
      .saveGuideResource(value, this.courseId)
      .subscribe((data: any) => {
