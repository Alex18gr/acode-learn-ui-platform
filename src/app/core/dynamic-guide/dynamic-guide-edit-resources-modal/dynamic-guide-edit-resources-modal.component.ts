...
export class DynamicGuideEditResourcesModalComponent implements OnInit {
  ...
  onAddResourcesTypeSelectChange(value: any) {
    ...
    this.resourceService
      .getAllCourseResources(this.courseId, value)
      .subscribe((data: any) => {
        console.log(data);
        this.addResourcesLoading = false;
        const resList = this.resourceService.getResourceListByResourceType(
          data.resources,
          value
        );
        this.addResourcesList =
          this.dynamicGuideService.getGuideResourcesFromResourcesList(resList);
      });
  }
}
